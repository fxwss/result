...
import { await_, err, fromFoundation, ok, try_ } from './result'
...
describe('Result API', () => {
	...
	describe('map', () => {
		...
		test('should chain multiple maps', () => {
			...
			expect(result.kind).toBe('ok')
			expect(result.unwrap()).toBe('Result: 10')
		})

		test('should propagate exceptions in map', () => {
			expect(() =>
				ok(5).map(() => {
	...
	describe('flatMap', () => {
		...
		test('should not flatMap an Err result', () => {
			const result = err<number, string>('initial error').flatMap((x) => ok(x * 2))
	...
	describe('mapErr', () => {
		...
		test('should transform error types', () => {
			const result = err<number, string>('error message').mapErr((msg) => new Error(msg))
	...
	describe('try_', () => {
		...
		test('should catch JSON parse errors', () => {
			const result = try_(() => JSON.parse('invalid json'))
	...
	describe('await_', () => {
		test('should wrap successful promise', async () => {
			const result = await await_(Promise.resolve(42))
		...
		test('should catch promise rejections', async () => {
			const result = await await_(Promise.reject(new Error('async error')))
		...
		test('should work with async/await', async () => {
			...
			const result = await await_(fetchData())

			expect(result.kind).toBe('ok')
			expect(result.unwrap()).toEqual({ data: 'success' })
		})

		test('should handle rejected promises with string', async () => {
			const result = await await_(Promise.reject('string error'))
	...
	describe('chaining operations', () => {
		...
		test('should work with real world example', () => {
			...
			const validateAge = (user: User) =>
				user.age >= 18 ? ok<User, string>(user) : err<User, string>('User is underage')
	...
	describe('toFoundation and fromFoundation', () => {
		...
		test('should convert from foundation', () => {
			const result = ok(42)
			const foundation = result.toFoundation()
			const converted = fromFoundation(foundation)

			expect(converted.kind).toBe('ok')
			expect(converted.unwrap()).toBe(42)
		})
